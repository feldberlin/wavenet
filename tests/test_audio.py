...
def test_load_raw_mono():
    y, sr = audio.load_raw("fixtures/goldberg/short.wav", mono=True)
    assert sr == 44100
    assert y.shape == (1, 414627)


def test_load_raw_stereo():
    y, sr = audio.load_raw("fixtures/goldberg/short.wav", mono=False)
    assert sr == 44100
...
def test_load_resampled_mono():
    ...
    y = audio.load_resampled("fixtures/goldberg/short.wav", p)
